...
class AnalyticsMethodsTest < ActionDispatch::IntegrationTest
   ...
   # End create_event_log tests
	...
	test "Can get event with log summaries of the last month" do
		matt = users(:matt)
		jwt = (JSON.parse login_user(matt, "schachmatt", devs(:sherlock)).body)["jwt"]
   ...
   # End get_event tests
	...
	test "Can't get event by name from outside the website" do
		matt = users(:matt)
		jwt = (JSON.parse login_user(matt, "schachmatt", devs(:matt)).body)["jwt"]
		event = events(:Login)

		get "/v1/analytics/event?name=#{event.name}&app_id=#{event.app_id}", headers: {Authorization: jwt}
		resp = JSON.parse(response.body)
	...
	test "Can't get event by name of the app of another dev" do
		matt = users(:matt)
		jwt = (JSON.parse login_user(matt, "schachmatt", devs(:sherlock)).body)["jwt"]
		event = events(:OpenApp)
	...
	test "Can get event by name with log summaries of the last month" do
		matt = users(:matt)
		jwt = (JSON.parse login_user(matt, "schachmatt", devs(:sherlock)).body)["jwt"]
		...
		get "/v1/analytics/event?name=#{event.name}&app_id=#{event.app_id}&period=2", headers: {Authorization: jwt}
		resp = JSON.parse(response.body)
	...
	test "Can get event by name with log summaries within specified timeframe" do
		...
		get "/v1/analytics/event?name=#{event.name}&app_id=#{event.app_id}&start=#{start_timestamp}&end=#{end_timestamp}", headers: {Authorization: jwt}
		resp = JSON.parse(response.body)
	...
	test "Can get event by name with log summaries sorted by hour" do
		...
		get "/v1/analytics/event?name=#{event.name}&app_id=#{event.app_id}&start=#{start_timestamp}&end=#{end_timestamp}&period=0", headers: {Authorization: jwt}
		resp = JSON.parse(response.body)
	...
	test "Can get event by name with log summaries sorted by month" do
		...
		get "/v1/analytics/event?name=#{event.name}&app_id=#{event.app_id}&start=#{start_timestamp}&end=#{end_timestamp}&period=2", headers: {Authorization: jwt}
		resp = JSON.parse(response.body)
	...
	test "Can get event by name with log summaries sorted by year" do
		...
		get "/v1/analytics/event?name=#{event.name}&app_id=#{event.app_id}&start=#{start_timestamp}&end=#{end_timestamp}&period=3", headers: {Authorization: jwt}
		resp = JSON.parse(response.body)
